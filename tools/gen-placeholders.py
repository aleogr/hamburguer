...
def build(name, label, size, base, seed, corner_label=False):
    ...
    if corner_label:
        # discreta, no rodape: essas imagens ficam atras de texto
        cfs = max(13, int(min(w, h) * 0.022))
        caption = (
            f'<g text-anchor="start" font-family="Helvetica Neue, Arial, sans-serif" '
            f'transform="translate({corner * 2}, {h - corner * 1.4:.0f})">'
